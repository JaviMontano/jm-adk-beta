...
version: 0.1.0
description: "Marca afirmaciones no verificables con tags de veracidad y propone el siguiente paso de verificacion."
owner: "JM Labs"
triggers:
  - revisor-veracidad
  - verificar-fuente
...
Mark every non-verifiable claim inline:
`{MEMORIA} {ADJUNTO} {EXTRAIDO_HILO} {WEB} {CONOCIMIENTO} {SUPUESTO} {INFERENCIA} {AUTOCOMPLETADO} {POR_CONFIRMAR} {VACIO_CRITICO}`.
For each `{SUPUESTO}`/`{POR_CONFIRMAR}`, propose the concrete next step that would verify it.
